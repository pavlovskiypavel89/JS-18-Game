...
// Добавляем словарь, и заполняем его объектами и препятствиями:
const actorDict = {
  'x': 'wallObsctacle', // x — стена, препятствие
  '!': 'lavaObstacle', // ! — лава, препятствие
  '@': Player, // @ — игрок, объект
  'o': Coin, // o — монетка, объект
  ...
  'v': FireRain  // v — огненный дождь, объект
}

// Добавляем парсер уровней игры из написанных схем:
const parser = new LevelParser(actorDict);

// Загружем написанные схемы (промис разрешится JSON-строкой, в которой закодирован массив схем написанных уровней),
// Парсим полученную JSON-строку (промис разрешится массивом строк - списком схем написанных уровней),
// Запускаем игру, передав в качестве аргументов: список схем уровней, парсер уровней и конструктор объекта, отвечающего за отрисовку игры в браузере (промис разрешится, когда игрок пройдет все уровни).
loadLevels()
